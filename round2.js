// Round 2 - Dynamic Spatial Attention Test with Audio & Polish
const qData = QuestionEngine.generateRound2();
...
if (cardDesc && qData.descriptionText) {
    cardDesc.textContent = qData.descriptionText;
}
...
const countdown = setInterval(function () {
    ...
    if (time <= 0) {
        ...
        if (cardTitle) cardTitle.textContent = "SPATIAL RECALL";
        if (cardDesc) cardDesc.textContent = "Quick! Pick the item in that position!";
...
optionsContainer.addEventListener("click", function (e) {
    ...
    QuestionEngine.showConfidencePopup(2, isCorrect, answer, qData.correctAnswer);
});